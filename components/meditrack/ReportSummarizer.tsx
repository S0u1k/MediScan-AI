...
export function ReportSummarizer() {
    ...
    try {
      ...
      if (data.available && data.raw) {
        ...
        try {
          ...
          saveActivityLog("report_summarized", "Report Summarizer", "Medical report summarized", { keyFindingsCount: result.keyFindings.length, warningsCount: result.warnings.length });
        } catch {
  ...
  return (
    <div className="space-y-6">
      ...
      {file && !summary && !error && (
        <GlassCard>
          <div className="mb-4 flex items-center justify-between"><h3 className="text-base font-medium text-white">Ready to Summarize</h3><button onClick={reset} className="flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1.5 text-sm text-white/80 hover:scale-105 active:scale-95 hover:bg-white/15 transition-all duration-300 ease-out"><X className="h-4 w-4" /> Cancel</button></div>
      ...
      {summary && (
        <>
          <GlassCard className="transition-all duration-300 ease-out hover:scale-[1.02] hover:-translate-y-1 hover:bg-white/10 hover:shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={reset} className="flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1.5 text-sm text-white/80 hover:scale-105 active:scale-95 hover:bg-white/15 transition-all duration-300 ease-out"><X className="h-4 w-4" /> New</button>
              </div>
            </div>
            <p className="text-sm text-white/80">{summary.shortSummary}</p>
          </GlassCard>
          {summary.keyFindings.length > 0 && <GlassCard className="transition-all duration-300 ease-out hover:scale-[1.02] hover:-translate-y-1 hover:bg-white/10 hover:shadow-2xl"><p className="mb-2 text-xs font-medium text-white/50">Key Findings</p><ul className="space-y-1">{summary.keyFindings.map((f, i) => <li key={i} className="flex items-start gap-2 text-sm text-white/80"><span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-green-400" />{f}</li>)}</ul></GlassCard>}
          {summary.warnings.length > 0 && <GlassCard className="transition-all duration-300 ease-out hover:scale-[1.02] hover:-translate-y-1 hover:bg-white/10 hover:shadow-2xl"><p className="mb-2 text-xs font-medium text-white/50">Warnings</p><ul className="space-y-1">{summary.warnings.map((w, i) => <li key={i} className="flex items-start gap-2 text-sm text-yellow-300/90"><AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" />{w}</li>)}</ul></GlassCard>}
          {summary.nextActions.length > 0 && <GlassCard className="transition-all duration-300 ease-out hover:scale-[1.02] hover:-translate-y-1 hover:bg-white/10 hover:shadow-2xl"><p className="mb-2 text-xs font-medium text-white/50">Next Actions</p><ul className="space-y-1">{summary.nextActions.map((a, i) => <li key={i} className="flex items-start gap-2 text-sm text-white/80"><span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-white/50" />{a}</li>)}</ul></GlassCard>}
          {summary.patientFriendly && <GlassCard className="transition-all duration-300 ease-out hover:scale-[1.02] hover:-translate-y-1 hover:bg-white/10 hover:shadow-2xl"><p className="mb-2 text-xs font-medium text-white/50">Patient-Friendly Explanation</p><p className="text-sm text-white/70">{summary.patientFriendly}</p></GlassCard>}
          {summary.doctorSummary && <GlassCard className="transition-all duration-300 ease-out hover:scale-[1.02] hover:-translate-y-1 hover:bg-white/10 hover:shadow-2xl"><p className="mb-2 text-xs font-medium text-white/50">Doctor Summary</p><p className="text-sm text-white/70 font-mono">{summary.doctorSummary}</p></GlassCard>}
        </>
      )}
    </div>
  );
